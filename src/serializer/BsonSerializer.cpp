...
char BsonSerializer::getBsonType(Json* ele) //throw Error
{
...
inline void BsonSerializer::writeDoc(BOStream& out, std::map<std::string, Json_t> eleList){
    ...
    for (auto& ite : eleList) {
        ...
        out << ite.first;
    	// print value
        writeBson(out, ite.second.get());
    }
    out << DOC_END;
}

void BsonSerializer::writeBson(BOStream& out, Json* ele){
    ...
    switch (ele->getType()) {
         ...
         default:
             throw Exception("Unsupported Json_t type : " + toString(ele->getType()));
     }
}

void BsonSerializer::write(std::ostream* out, Json* data, EncodingOption flag){
    BOStream str(out);
    switch (data->getType()) {
        case JSON_ARRAY:
        case JSON_OBJECT:
        ...
        default:
            throw Exception("Bson require that root object is either an object or an array. Is an " + toString(data->getType()));
            break;
    }
}
...
public :
    ...
    void throwException(const std::string& what){
        throw Exception(what + " at location " + toString((int)_in->tellg()));
    }
};
        ...
        case ELE_TYPE_ARRAY:
        {
            ...
            while (type != EOF && type != DOC_END){
                ...
                if (match && deepness+1 == parts.size())
                    retVal.push_back(readBson(in, type));

                else if (match) {
                    auto subRet = searchBson(in, type, deepness+inc, parts);
...
std::vector<Json_t> BsonSerializer::extract(std::istream* in, DecodingOption flag, const std::string& path){
    BIStream str(in);
    return searchBson(str, ELE_TYPE_OBJECT, 0, tokenize(path, "/"));
}
